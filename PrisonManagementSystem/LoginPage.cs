...
namespace PrisonManagementSystem
{
    public partial class LoginPage : Form
    {
        ...
        {
            string sql = "select * from LoginTable where id = '" + this.txt_id_loginPage.Text + "' and password = '" + this.txt_pass_loginPage.Text + "';";
            DatabaseConfiguration dbc = new DatabaseConfiguration();
            DataSet ds = dbc.ExecuteQuery(sql);

            if (ds.Tables[0].Rows.Count == 1 && this.txt_id_loginPage.Text.Contains("AM"))
            {
            ...
            else if(ds.Tables[0].Rows.Count == 1 && this.txt_id_loginPage.Text.Contains("SV"))
            {
                MessageBox.Show("Login Approved For " + ds.Tables[0].Rows[0][3].ToString());
                MessageBox.Show("Login Approved For " + ds.Tables[0].Rows[0][1].ToString());
                this.Hide();
